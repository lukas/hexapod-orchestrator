...
def _track_badge(tid: str, text: str, recent: list[dict],
    ...
    # Current lifecycle overrides historical prose; activity comes only
    # from the ledger, never from old 'launched/running' journal entries.
    if lifecycle == "open":
        return "IDLE / OPEN", "open"
    if lifecycle == "retired" or (not lifecycle and (
            "retired" in registry_status.lower() or re.match(
...
def _track_where(tid: str, badge: str, recent: list[dict],
    ...
    if badge == "IDLE / OPEN":
        return "Research remains open; no training run or pending run analysis in the current ledger."
...
def research_brief(f: dict, pending: dict[str, str]) -> dict:
    ...
    for order, tid in enumerate(keys):
        ...
        badge, cls = _track_badge(
            ...
            track_meta.get("lifecycle", ""))
        name = track_meta.get("name") or d.get("name", tid)
        result = latest_research_result(text)
        where = _track_where(tid, badge, recent, pending)
        queued = [e.get("run", "") for e in f.get("backlog", {}).get("queued", [])
                  if track_of_entry(e) == tid]
        if queued:
            if badge not in ("ACTIVE NOW", "ANALYZING"):
                badge, cls = "QUEUED", "open"
                where = "No training run or pending run analysis in the current ledger."
            where += " Queued: " + ", ".join(queued[:3]) + "."
        if cls == "active":
...
def render_research_brief(brief: dict) -> list[str]:
    ...
    for t in brief.get("topics", []):
        h.append(f"<article class='topic {esc(t.get('cls', 'watch'))}'>"
                 ...
                 f"<a class='tracklink' href='/track/{urllib.parse.quote(t['id'], safe='')}'>{esc(t['name'])}</a></span><span class='topicid'>"
                 ...
                 f"<a href='/track/{urllib.parse.quote(t['id'], safe='')}'>Experiments &amp; spend "
                 f"&#8594;</a> <a href='/track/{urllib.parse.quote(t['id'], safe='')}#videos'>▶ Watch MuJoCo</a>")
...
def render_track_page(tid: str, page: int = 1) -> str | None:
    import track_page
    return track_page.render(sys.modules[__name__], tid, page)
    ...
    def do_GET(self):  # noqa: N802
        ...
        if is_llm:
            ...
            if out is None:
                ...
                return
            body, ctype = out
        elif u.path.startswith(("/cycle/", "/run/", "/track/")):
            from urllib.parse import unquote
            q = parse_qs(u.query)
            if u.path.startswith("/cycle/"):
                page = render_cycle_page(
                    ...
                    q.get("part", ["log"])[0],
                    q.get("full", ["0"])[0] == "1")
            elif u.path.startswith("/track/"):
                try:
                    page_number = int(q.get("page", ["1"])[0])
                except ValueError:
                    page_number = 1
                page = render_track_page(unquote(u.path[len("/track/"):]).strip("/"), page_number)
            else:
                page = render_run_page(
                    unquote(u.path[len("/run/"):]).strip("/"))
            if page is None:
                body = b"404: no such cycle/run/track"
                self.send_response(404)
                self.send_header("Content-Type", "text/plain")
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
